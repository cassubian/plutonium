...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uPUK
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	// 18315 is the block of our rebranding and relaunching
    if (nHeight >= 18315) nSubsidy = 500 * COIN;
    if (nHeight >= 22635) nSubsidy = 50 * COIN;
...
void ThreadScriptCheck() {
    RenameThread("plutoniumkoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1390691840, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x617f7a22 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x61, 0x7f, 0x7a, 0x22 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PlutoniumKoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PlutoniumKoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PlutoniumKoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("plutoniumkoin-miner");
    ...
    try { loop {
        ...
        printf("Running PlutoniumKoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PlutoniumKoinMiner terminated\n");
        throw;
    }
}
