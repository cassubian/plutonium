...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("PUK")
    updateDisplayUnit();
}
...
{
    QString ret = QString("plutoniumkoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PUK unit input in lnReqAmount, we generate the URI with PUK as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PUK, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
